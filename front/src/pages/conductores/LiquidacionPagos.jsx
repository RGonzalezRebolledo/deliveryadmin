...
function LiquidacionRepartidoresAdmin() {
  ...
  // Filtrado dinámico por búsqueda (Nombre, Cédula o Código)
  const filteredDrivers = drivers.filter((d) => {
    const query = searchTerm.toLowerCase();
    const nombreCompleto = `${d.nombre || ""} ${d.apellido || ""}`.toLowerCase();
    const cedula = (d.cedula || "").toLowerCase();
    const codigo = (d.codigo_conductor || "").toLowerCase();

    return nombreCompleto.includes(query) || cedula.includes(query) || codigo.includes(query);
  });
        ...
        >
          ...
          >
            ...
            <div style={{ position: "relative", flex: 3, minWidth: "220px" }}>
              <input
                type="text"
                placeholder="Buscar por código, nombre o cédula del conductor..."
                value={searchTerm}
          ...
          <table className="admin-table">
            <thead>
              <tr>
                ...
                <th style={{ textAlign: "center" }}>Número de Cuenta</th>
